# eines_automation_v6_5c.py
# - Basado en v6_5a, con:
#   * Detección robusta de cabecera en Excel (evita "Unnamed")
#   * Mapeo flexible de columnas (Station/Camera/Frame/X_FRAME/Y_FRAME)
#   * OCR tras maximizar para ubicar Mesh/Station/Camera/Frame/PosX/PosY/Show
#   * Selección de Mesh -> Station -> Camera y envío de Frame/X/Y
#   * Guardado de X_NEW/Y_NEW/Z_NEW/TIME_MS
#   * --dry para ver mapeo sin tocar la UI
...
def load_excel_any_header(path, sheet_name="Hoja1"):
    ...
    # ensure output cols exist as object
...
def find_footer_labels(dlg):
    ...
    for offset in range(220, 80, -8):
        ...
        if data:
            for i,txt in enumerate(data["text"]):
                t=(txt or "").strip()
                if not t: continue
                for key,wanted in {"MESH":"MESH","STATION":"STATION","CAMERA":"CAMERA","FRAME":"FRAME","POSITIONX":"POSITION","POSITIONY":"POSITION","SHOW":"SHOW"}.items():
                    if t.upper().startswith(wanted):
                        x=rect[0]+data["left"][i]; y=rect[1]+data["top"][i]; w=data["width"][i]; h=data["height"][i]
                        labels[key]=(x,y,w,h)
        if len(labels)>best_score:
            best_labels, best_rect, best_score = labels, rect, len(labels)
            if best_score>=6: break
    # Convert POSITIONX/Y to POSX/POSY
    labels_final={}
    for k,v in best_labels.items():
        if k=="POSITIONX": labels_final["POSX"]=v
        elif k=="POSITIONY": labels_final["POSY"]=v
        else: labels_final[k]=v
    # default approx if some missing
    ...
    for k,(x,y,w,h) in labels_final.items():
...
def main():
    ...
    ap.add_argument("--config", default="config_v6_5c.json")
    ...
    # --- Launch viewer and detect footer ---
    ...
    labels, rect = find_footer_labels(dlg)
    ...
    for idx,row in df.iterrows():
        try:
            ...
            if not st_name:
                print(f"[WARN] Row {idx}: unknown station {st_id}"); continue
            ...
            fx,fy = click_right_of(labels["FRAME"], dx=150); put_text_at(fx,fy,row[col_frame])
            px,py_ = click_right_of(labels["POSX"],  dx=150); put_text_at(px,py_,row[col_xf])
            pyx,pyy= click_right_of(labels["POSY"],  dx=150); put_text_at(pyx,pyy,row[col_yf])
    ...
    try:
        with pd.ExcelWriter(out, engine="openpyxl", mode="w") as w: df.to_excel(w, sheet_name=cfg.get("excel_sheet","Hoja1"), index=False)
        print(f"[DONE] Saved to {out}")
    except PermissionError:
        alt=str(Path(out).with_name(Path(out).stem+"_out.xlsx"))
        with pd.ExcelWriter(alt, engine="openpyxl", mode="w") as w: df.to_excel(w, sheet_name=cfg.get("excel_sheet","Hoja1"), index=False)
