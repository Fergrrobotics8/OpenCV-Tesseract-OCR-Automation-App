...
def main():
    ...
    for idx, row in df.iterrows():
        try:
            ...
            station_id = int(row[col_station])
            camera_id = int(row[col_camera])
            frame = str(int(float(row[col_frame])))
            x_val = str(int(float(row[col_xf])))
            y_val = str(int(float(row[col_yf])))
