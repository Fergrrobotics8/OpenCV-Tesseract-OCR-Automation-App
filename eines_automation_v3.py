...
def wait_until_enabled(ctrl, timeout=15.0):
    t0=time.time()
...
def click_in_window(dlg):
    r = dlg.rectangle()
    pyautogui.click(r.left+120, r.top+120)

def select_mesh(ctrls, mesh_name):
    if "mesh" not in ctrls: return
    try:
        ctrls["mesh"].select(mesh_name)
    except Exception:
        rect = ctrls["mesh"].rectangle()
...
def ocr_intersect_line(dlg, ctrls):
    try:
        txt = ctrls.get("intersect_text", None)
        if txt:
            rect = txt.rectangle()
...
def main():
    ...
    ap.add_argument("--config", default="config_v3.json")
    ...
    # Wait for enabling
    for key in ["station","camera","show"]:
        if key in ctrls and not wait_until_enabled(ctrls[key], timeout=20.0):
            print(f"[WARN] Control '{key}' not enabled after waiting.")
    ...
    for idx,row in df.iterrows():
        try:
            ...
            text = ocr_intersect_line(dlg, ctrls)
