...
def load_excel(path):
    ...
    # Keep original names, but create a normalized mapping
    cols = list(df.columns)
    # ensure outputs as object
    for c in ["X_NEW","Y_NEW","Z_NEW","TIME_MS"]:
        if c not in df.columns: df[c] = ""
        df[c] = df[c].astype("object")
    return df, sheet, cols
...
def find_footer_labels(dlg):
                ...
                for key,wanted in {"MESH":"MESH","STATION":"STATION","CAMERA":"CAMERA","FRAME":"FRAME","POSITIONX":"POSITION","POSITIONY":"POSITION","SHOW":"SHOW"}.items():
                    if t.upper().startswith(wanted):
                        ...
                        if key in ["POSITIONX","POSITIONY"]:
                            # disambiguate X/Y by looking at the next text
                            labels[key]=(x,y,w,h)
                        else:
                            labels[key]=(x,y,w,h)
    ...
    # Convert POSITIONX/Y to POSX/POSY (rough split: X above Y)
    labels_final={}
...
def main():
    ...
    ap.add_argument("--config", default="config_v6_5a.json")
    ...
    df, sheet, orig_cols = load_excel(cfg["excel_path"])
    # robust column picking
    col_station = pick_col(df, ["STATION","STATIONS","STATIONID"])
    col_camera  = pick_col(df, ["CAMERA","CAM","CAMERAID"])
    col_frame   = pick_col(df, ["FRAME"])
    col_xf      = pick_col(df, ["XFRAME","X_FRM","XCOORD","X"])
    col_yf      = pick_col(df, ["YFRAME","Y_FRM","YCOORD","Y"])
    ...
    labels, rect = find_footer_labels(dlg)  # OCR post-maximize
    # Mesh con upshifts
    ...
    nr_col = pick_col(df, ["NR","NR.","Nº","N°","NO","NUM","NUMERO"]) or df.columns[0]
